//              WARNING
// This file is an empty shell containing only public C# items.
// The internal code is hidden; to access it, you need to obtain a Source licence of the library.

using System;

namespace Rtde {

	public class RtdeIntRegistersValue : RtdeRegistersValue<int>, IRtdeRegistersValue {


		private RtdeIntRegistersValue() : base(default)
		{
			// Source is hidden, a Source licence is needed to access internal code...
		}

		/// <summary>
		/// Register n°0
		/// </summary>
		public int X0 { get; set; }

		/// <summary>
		/// Register n°1
		/// </summary>
		public int X1 { get; set; }

		/// <summary>
		/// Register n°2
		/// </summary>
		public int X2 { get; set; }

		/// <summary>
		/// Register n°3
		/// </summary>
		public int X3 { get; set; }

		/// <summary>
		/// Register n°4
		/// </summary>
		public int X4 { get; set; }

		/// <summary>
		/// Register n°5
		/// </summary>
		public int X5 { get; set; }

		/// <summary>
		/// Register n°6
		/// </summary>
		public int X6 { get; set; }

		/// <summary>
		/// Register n°7
		/// </summary>
		public int X7 { get; set; }

		/// <summary>
		/// Register n°8
		/// </summary>
		public int X8 { get; set; }

		/// <summary>
		/// Register n°9
		/// </summary>
		public int X9 { get; set; }

		/// <summary>
		/// Register n°10
		/// </summary>
		public int X10 { get; set; }

		/// <summary>
		/// Register n°11
		/// </summary>
		public int X11 { get; set; }

		/// <summary>
		/// Register n°12
		/// </summary>
		public int X12 { get; set; }

		/// <summary>
		/// Register n°13
		/// </summary>
		public int X13 { get; set; }

		/// <summary>
		/// Register n°14
		/// </summary>
		public int X14 { get; set; }

		/// <summary>
		/// Register n°15
		/// </summary>
		public int X15 { get; set; }

		/// <summary>
		/// Register n°16
		/// </summary>
		public int X16 { get; set; }

		/// <summary>
		/// Register n°17
		/// </summary>
		public int X17 { get; set; }

		/// <summary>
		/// Register n°18
		/// </summary>
		public int X18 { get; set; }

		/// <summary>
		/// Register n°19
		/// </summary>
		public int X19 { get; set; }

		/// <summary>
		/// Register n°20
		/// </summary>
		public int X20 { get; set; }

		/// <summary>
		/// Register n°21
		/// </summary>
		public int X21 { get; set; }

		/// <summary>
		/// Register n°22
		/// </summary>
		public int X22 { get; set; }

		/// <summary>
		/// Register n°23
		/// </summary>
		public int X23 { get; set; }

		/// <summary>
		/// Register n°24
		/// </summary>
		public int X24 { get; set; }

		/// <summary>
		/// Register n°25
		/// </summary>
		public int X25 { get; set; }

		/// <summary>
		/// Register n°26
		/// </summary>
		public int X26 { get; set; }

		/// <summary>
		/// Register n°27
		/// </summary>
		public int X27 { get; set; }

		/// <summary>
		/// Register n°28
		/// </summary>
		public int X28 { get; set; }

		/// <summary>
		/// Register n°29
		/// </summary>
		public int X29 { get; set; }

		/// <summary>
		/// Register n°30
		/// </summary>
		public int X30 { get; set; }

		/// <summary>
		/// Register n°31
		/// </summary>
		public int X31 { get; set; }

		/// <summary>
		/// Register n°32
		/// </summary>
		public int X32 { get; set; }

		/// <summary>
		/// Register n°33
		/// </summary>
		public int X33 { get; set; }

		/// <summary>
		/// Register n°34
		/// </summary>
		public int X34 { get; set; }

		/// <summary>
		/// Register n°35
		/// </summary>
		public int X35 { get; set; }

		/// <summary>
		/// Register n°36
		/// </summary>
		public int X36 { get; set; }

		/// <summary>
		/// Register n°37
		/// </summary>
		public int X37 { get; set; }

		/// <summary>
		/// Register n°38
		/// </summary>
		public int X38 { get; set; }

		/// <summary>
		/// Register n°39
		/// </summary>
		public int X39 { get; set; }

		/// <summary>
		/// Register n°40
		/// </summary>
		public int X40 { get; set; }

		/// <summary>
		/// Register n°41
		/// </summary>
		public int X41 { get; set; }

		/// <summary>
		/// Register n°42
		/// </summary>
		public int X42 { get; set; }

		/// <summary>
		/// Register n°43
		/// </summary>
		public int X43 { get; set; }

		/// <summary>
		/// Register n°44
		/// </summary>
		public int X44 { get; set; }

		/// <summary>
		/// Register n°45
		/// </summary>
		public int X45 { get; set; }

		/// <summary>
		/// Register n°46
		/// </summary>
		public int X46 { get; set; }

		/// <summary>
		/// Register n°47
		/// </summary>
		public int X47 { get; set; }


		public override int LowerRangeIndex => default;
	}
}
