//              WARNING
// This file is an empty shell containing only public C# items.
// The internal code is hidden; to access it, you need to obtain a Source licence of the library.

using System;

namespace Rtde {
	/// <summary>
	/// RTDE double register array (48 registers, indices 0–47) for exchanging 64-bit floating-point values with the robot.
	/// </summary>
	public class RtdeDoubleRegistersValue : RtdeRegistersValue<double>, IRtdeRegistersValue {


		private RtdeDoubleRegistersValue() : base(default)
		{
			// Source is hidden, a Source licence is needed to access internal code...
		}

		/// <summary>
		/// Register n°0
		/// </summary>
		public double X0 { get; set; }

		/// <summary>
		/// Register n°1
		/// </summary>
		public double X1 { get; set; }

		/// <summary>
		/// Register n°2
		/// </summary>
		public double X2 { get; set; }

		/// <summary>
		/// Register n°3
		/// </summary>
		public double X3 { get; set; }

		/// <summary>
		/// Register n°4
		/// </summary>
		public double X4 { get; set; }

		/// <summary>
		/// Register n°5
		/// </summary>
		public double X5 { get; set; }

		/// <summary>
		/// Register n°6
		/// </summary>
		public double X6 { get; set; }

		/// <summary>
		/// Register n°7
		/// </summary>
		public double X7 { get; set; }

		/// <summary>
		/// Register n°8
		/// </summary>
		public double X8 { get; set; }

		/// <summary>
		/// Register n°9
		/// </summary>
		public double X9 { get; set; }

		/// <summary>
		/// Register n°10
		/// </summary>
		public double X10 { get; set; }

		/// <summary>
		/// Register n°11
		/// </summary>
		public double X11 { get; set; }

		/// <summary>
		/// Register n°12
		/// </summary>
		public double X12 { get; set; }

		/// <summary>
		/// Register n°13
		/// </summary>
		public double X13 { get; set; }

		/// <summary>
		/// Register n°14
		/// </summary>
		public double X14 { get; set; }

		/// <summary>
		/// Register n°15
		/// </summary>
		public double X15 { get; set; }

		/// <summary>
		/// Register n°16
		/// </summary>
		public double X16 { get; set; }

		/// <summary>
		/// Register n°17
		/// </summary>
		public double X17 { get; set; }

		/// <summary>
		/// Register n°18
		/// </summary>
		public double X18 { get; set; }

		/// <summary>
		/// Register n°19
		/// </summary>
		public double X19 { get; set; }

		/// <summary>
		/// Register n°20
		/// </summary>
		public double X20 { get; set; }

		/// <summary>
		/// Register n°21
		/// </summary>
		public double X21 { get; set; }

		/// <summary>
		/// Register n°22
		/// </summary>
		public double X22 { get; set; }

		/// <summary>
		/// Register n°23
		/// </summary>
		public double X23 { get; set; }

		/// <summary>
		/// Register n°24
		/// </summary>
		public double X24 { get; set; }

		/// <summary>
		/// Register n°25
		/// </summary>
		public double X25 { get; set; }

		/// <summary>
		/// Register n°26
		/// </summary>
		public double X26 { get; set; }

		/// <summary>
		/// Register n°27
		/// </summary>
		public double X27 { get; set; }

		/// <summary>
		/// Register n°28
		/// </summary>
		public double X28 { get; set; }

		/// <summary>
		/// Register n°29
		/// </summary>
		public double X29 { get; set; }

		/// <summary>
		/// Register n°30
		/// </summary>
		public double X30 { get; set; }

		/// <summary>
		/// Register n°31
		/// </summary>
		public double X31 { get; set; }

		/// <summary>
		/// Register n°32
		/// </summary>
		public double X32 { get; set; }

		/// <summary>
		/// Register n°33
		/// </summary>
		public double X33 { get; set; }

		/// <summary>
		/// Register n°34
		/// </summary>
		public double X34 { get; set; }

		/// <summary>
		/// Register n°35
		/// </summary>
		public double X35 { get; set; }

		/// <summary>
		/// Register n°36
		/// </summary>
		public double X36 { get; set; }

		/// <summary>
		/// Register n°37
		/// </summary>
		public double X37 { get; set; }

		/// <summary>
		/// Register n°38
		/// </summary>
		public double X38 { get; set; }

		/// <summary>
		/// Register n°39
		/// </summary>
		public double X39 { get; set; }

		/// <summary>
		/// Register n°40
		/// </summary>
		public double X40 { get; set; }

		/// <summary>
		/// Register n°41
		/// </summary>
		public double X41 { get; set; }

		/// <summary>
		/// Register n°42
		/// </summary>
		public double X42 { get; set; }

		/// <summary>
		/// Register n°43
		/// </summary>
		public double X43 { get; set; }

		/// <summary>
		/// Register n°44
		/// </summary>
		public double X44 { get; set; }

		/// <summary>
		/// Register n°45
		/// </summary>
		public double X45 { get; set; }

		/// <summary>
		/// Register n°46
		/// </summary>
		public double X46 { get; set; }

		/// <summary>
		/// Register n°47
		/// </summary>
		public double X47 { get; set; }

		/// <summary>
		/// Gets the lower-bound register index for this register range.
		/// </summary>
		public override int LowerRangeIndex => default;
	}
}
