...
from UnderAutomation.UniversalRobots.Ssh.Tools.Security.Cryptography.Ciphers import SerpentCipher as serpent_cipher

class SerpentCipher(BlockCipher):
	def __init__(self, key: int, mode: CipherMode, padding: CipherPadding, _internal = 0):
		if(_internal == 0):
			self._instance = serpent_cipher(key, mode, padding)
		else:
