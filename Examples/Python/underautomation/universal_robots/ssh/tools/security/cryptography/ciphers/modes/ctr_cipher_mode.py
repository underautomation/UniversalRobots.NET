...
from UnderAutomation.UniversalRobots.Ssh.Tools.Security.Cryptography.Ciphers.Modes import CtrCipherMode as ctr_cipher_mode

class CtrCipherMode(CipherMode):
	def __init__(self, iv: typing.List[int], _internal = 0):
		if(_internal == 0):
			self._instance = ctr_cipher_mode(iv)
		else:
