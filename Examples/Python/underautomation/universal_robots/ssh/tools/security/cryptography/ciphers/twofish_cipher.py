...
from UnderAutomation.UniversalRobots.Ssh.Tools.Security.Cryptography.Ciphers import TwofishCipher as twofish_cipher

class TwofishCipher(BlockCipher):
	def __init__(self, key: typing.List[int], mode: CipherMode, padding: CipherPadding, _internal = 0):
		if(_internal == 0):
			self._instance = twofish_cipher(key, mode, padding)
		else:
