...
from UnderAutomation.UniversalRobots.Ssh.Tools.Messages.Connection import ChannelCloseMessage as channel_close_message

class ChannelCloseMessage(ChannelMessage):
	def __init__(self, localChannelNumber: int, _internal = 0):
		if(_internal == 0):
			self._instance = channel_close_message(localChannelNumber)
		else:
