...
class SshDataStream:
	def __init__(self, buffer: int, offset: int, count: int, _internal = 0):
		if(_internal == 0):
	...
	def read_binary(self) -> int:
		return self._instance.ReadBinary()
	def write_binary(self, buffer: int) -> None:
		self._instance.WriteBinary(buffer)
	def write_binary(self, buffer: int, offset: int, count: int) -> None:
